...
syntax match mustacheConditionals '\([/#]\?\<\(if\|ifeq\|unless\)\|\<else\)\>' contained containedin=@mustacheInside
syntax match mustacheHelpers '[/#]\?\<\(with\|link\-to\|each\(\-in\)\?\|let\)\>' contained containedin=@mustacheInside
syntax match mustacheHelpers 'else \(if\|ifeq\|unless\|with\|link\-to\|each\(\-in\)\?\)' contained containedin=@mustacheInside
